...
"""
Modified from jointcontroller_host.py from robot RR. Adds workspace controller.
...
function void setWorkspaceCommand(string limb, double[] delt_pos, double[] rot)
...
class Baxter_impl(object):
    def __init__(self):
        ...
        self.kin_l = baxter_kinematics('left')
        self.kin_r = baxter_kinematics('right')
    ...
    @property
    def JacobianTransposeWrenches(self):
        return self.JTW
    ...
    def setWorkspaceCommand(self, limb, delt_pos, rot):
        """
        sets end-effector to desired position and orientation
        INPUTS:
        string limb: name of limb 'right' or 'left'
        double[] delt_pos: change in cartesian position
        double[] rot: quaternion orientation
        """
        if limb == 'right':
            pos_i = self.kin_r.forward_position_kinematics()
            pos_i = pos_i[0:3]
            pos = pos_i+delt_pos
            joints = self.kin_r.inverse_kinematics(pos.tolist(), rot.tolist())
            if joints is None:
                print "Cannot Reach Desired Position"
            else:
                self.setJointCommand(limb, joints)
        elif limb == 'left':
            pos_i = self.kin_l.forward_position_kinematics()
            pos_i = pos_i[0:3]

            pos = pos_i+delt_pos
            pos = fk_new = map(add, pos_i.tolist(), delt_pos.tolist())
            joints = self.kin_l.inverse_kinematics(pos, rot.tolist())
            if joints is None:
                print "Cannot Reach Desired Position"
            else:
                self.setJointCommand(limb, joints)
        else:
            print "Limb not Valid"
...
def main(argv):
    ...
    parser = argparse.ArgumentParser(
                            description='Initialize Workspace Controller.')
    ...
    #Set the Node name
    RR.RobotRaconteurNode.s.NodeName="BaxterWorkspaceServer"
    ...
    print "tcp://localhost:" + str(port) + "/BaxterWorkspaceServer/Baxter"
